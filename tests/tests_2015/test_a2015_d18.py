...
def test_cell_which_is_off_and_has_less_than_3_neighbors_stays_off():
    ...
    assert Vector2D(1, 1) not in game.next_live_cells(set())
    assert Vector2D(1, 1) not in game.next_live_cells({Vector2D(0, 0)})
    assert Vector2D(1, 1) not in game.next_live_cells({Vector2D(0, 0), Vector2D(0, 1)})


def test_cell_which_is_off_and_has_more_than_3_neighbors_stays_off():
    game = GameOfLifeLights(3, 3)
    assert Vector2D(1, 1) not in game.next_live_cells(
...
def test_cell_which_is_on_and_has_less_than_two_neighbors_dies():
    game = GameOfLifeLights(3, 3)
    assert Vector2D(1, 1) not in game.next_live_cells({Vector2D(1, 1)})
    assert Vector2D(1, 1) not in game.next_live_cells({Vector2D(1, 1), Vector2D(0, 0)})
...
def test_cell_which_is_on_and_has_two_or_three_neighbors_survives():
    game = GameOfLifeLights(3, 3)
    assert Vector2D(1, 1) in game.next_live_cells(
...
def test_cell_outside_grid_never_comes_alive():
    game = GameOfLifeLights(3, 3)
    assert (3, 1) not in game.next_live_cells(
