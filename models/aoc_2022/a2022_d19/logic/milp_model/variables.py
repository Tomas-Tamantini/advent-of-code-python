    ...
    for resource_type in ResourceType:
        for minute in range(1, time_limit + 2):
            ...
            yield ContinuousVariable(
                ...
                description=f"Amount of resource {resource_type} at the start of minute {minute}",
                lower_bound=0,
            )

            yield ContinuousVariable(
                id=VariableId(VariableClass.FLEET_SIZE, resource_type, minute),
                description=f"Fleet size of robot type {resource_type} at the start of minute {minute}",
                lower_bound=0,
            )
