...
def test_next_navigator_on_board_wraps_around_horizontally_if_off_edge():
    rows = (Interval(8, 10), Interval(8, 10), Interval(9, 20))
    board = ObstacleBoard(rows, wall_positions=set())
    navigator = BoardNavigator(position=Vector2D(11, 2), facing=CardinalDirection.EAST)
    next_navigator = board.move_navigator_forward(navigator, num_steps=30)
    assert next_navigator.position == Vector2D(17, 2)

    navigator = BoardNavigator(position=Vector2D(11, 2), facing=CardinalDirection.WEST)
    next_navigator = board.move_navigator_forward(navigator, num_steps=4)
    assert next_navigator.position == Vector2D(19, 2)


def test_next_navigator_on_board_wraps_around_vertically_if_off_edge():
    ...
    next_navigator = board.move_navigator_forward(navigator, num_steps=5)
    assert next_navigator.position == Vector2D(9, 3)


def test_next_navigator_on_board_is_just_before_obstacle_if_one_is_on_the_way():
    rows = (Interval(0, 100),)
