...
def test_guarded_bathroom_counts_number_of_guards_per_quadrant():
    ...
    num_guards_per_quadrant = sorted(
        bathroom.num_guards_per_quadrant(_initial_guards, time=100)
    )
    assert num_guards_per_quadrant == [1, 1, 3, 4]


def test_guarded_bathroom_renders_guard_positions():
    bathroom = GuardedBathroom(width=11, height=7)
    expected = (
        "......#..#.\n"
        "...........\n"
    ...
    assert bathroom.render_guards_at_time(_initial_guards, time=100) == expected
