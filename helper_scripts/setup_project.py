...
def _create_solution_file(
    ...
    with open(os.path.join(solution_path, "solution.py"), "w", encoding="utf-8") as f:
        ...
        f.write(
            f"\n\ndef aoc_{year}_d{day}(io_handler: IOHandler)"
            " -> Iterator[ProblemSolution]:\n"
        )
...
def _create_test_folder(test_path: str) -> None:
    ...
    with open(os.path.join(test_path, "__init__.py"), "w", encoding="utf-8") as _:
        ...
...
def _create_parser_file(solution_path: str, parser_method_name: str) -> None:
    with open(os.path.join(solution_path, "parser.py"), "w", encoding="utf-8") as f:
        f.write("from models.common.io import InputReader\n")
        f.write("\n\n")
        f.write(
            f"def {parser_method_name}(input_reader: InputReader)"
            ' -> "InsertReturnType":\n'
        )
        f.write("    pass\n")
...
def _create_parser_test_file(parser_method_name: str, test_path: str) -> None:
    with open(os.path.join(test_path, "test_parser.py"), "w", encoding="utf-8") as f:
...
def _create_empty_input_file(year: int, day: int) -> None:
    with open(
        os.path.join("files", "input_files", f"aoc_{year}", f"a{year}_d{day}.txt"),
        "w",
        encoding="utf-8",
    ) as f:
        f.write("")
